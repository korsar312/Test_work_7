<template>
  <aside class="container wrapperCard">
    <div class="remove" v-on:click="remove()">
...
export default {
  ...
  computed: {
    thousandthSpace () {
      ...
      if (price.includes('.')) {
        return parseFloat(price.replace(/[^\d]/g, '').replace(/(\d\d?)$/, '.$1')).toFixed(2)
      }
      return price
        .split('').reverse().join('')
        .replace(/(.{3})/g, '$1 ')
